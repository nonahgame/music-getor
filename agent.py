...
# -----------------------------
# TEXT SANITIZER FOR FFMPEG
# -----------------------------
def escape_ffmpeg_text(t: str) -> str:
    """
    Escapes characters that break ffmpeg drawtext:
    - Single quotes
    - Newlines
    - Backslashes
    """
...
def upload_to_github(local_file_path, github_folder="d-output"):
    """ Upload generated music/video to GitHub repo """
    try:
...
def run_agent(data):
    ...
    safe_lyrics = escape_ffmpeg_text(lyrics)
    short_lyrics = escape_ffmpeg_text(lyrics[:300])
    ...
    "[0:a]showwaves=s=1280x200:colors=#00ff00:mode=cline[v];\
     ...
     drawtext=text='{short_lyrics}':fontcolor=yellow:fontsize=40:x=(w-text_w)/2:y=h-th-150" \
